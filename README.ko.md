...
AI 연구는 방대한 고차원 데이터 — 임베딩, 활성화, 어텐션 맵 — 를 생성하지만, 거의 모든 사람이 평면 2D 플롯으로만 봅니다. 이 도구는 임베딩 공간을 탐색 가능한 3D 세계로 렌더링하며, ParaView와 동일한 툴킷으로 구축되었습니다. 시작 시 **실시간** `all-MiniLM-L6-v2` 개념 아틀라스를 로드합니다(첫 실행 ~25MB). 직접 입력한 단어 임베딩이나 파일 업로드도 가능합니다.
